...
public class HistoryFragmentViewModel extends BaseViewModel<HistoryFragmentNavigator> {
    ...
    public void fetchNewOrder(Activity mActivity) {
        if (NetworkUtils.isNetworkConnected(mActivity)) {
            getmDataManger().fetchNewOrder(mActivity, getmDataManger().getAccessToken(), new ResponseListener() {
                @Override
                public void onSuccess(JsonObject jsonObject) {
                    try {
                        getmNavigator().get().hideProgressBars();
                        getmNavigator().get().onSuccessNewOrder(jsonObject);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
    ...
    public void saveRating(Activity mActivity, int order_id, int store_id, float rating, int position) {
        if (NetworkUtils.isNetworkConnected(mActivity)) {
            getmNavigator().get().showProgressBars();
            getmDataManger().saveRating(mActivity, getmDataManger().getAccessToken(), order_id, store_id, rating, new ResponseListener() {
                @Override
                public void onSuccess(JsonObject jsonObject) {
                    try {
                        getmNavigator().get().hideProgressBars();
                        getmNavigator().get().onSuccessSaveRating(jsonObject, position, rating);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
