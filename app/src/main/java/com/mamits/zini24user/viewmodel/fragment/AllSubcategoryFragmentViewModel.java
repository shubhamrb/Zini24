...
public class AllSubcategoryFragmentViewModel extends BaseViewModel<AllSubcategoryFragmentNavigator> {

    public AllSubcategoryFragmentViewModel(IDataManager dataManager, ISchedulerProvider schedulerProvider) {
        super(dataManager, schedulerProvider);
    }

    public void fetchSubcategory(Activity mActivity, JSONObject jsonObject) {
        if (NetworkUtils.isNetworkConnected(mActivity)) {
            getmNavigator().get().showProgressBars();
            getmDataManger().fetchSubcategory(mActivity,getmDataManger().getAccessToken(), jsonObject, new ResponseListener() {
                @Override
                public void onSuccess(JsonObject jsonObject) {
                    try {
                        getmNavigator().get().hideProgressBars();
                        getmNavigator().get().onSuccessSubcategory(jsonObject);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
