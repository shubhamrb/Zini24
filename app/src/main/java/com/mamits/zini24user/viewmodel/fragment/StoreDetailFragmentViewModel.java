...
public class StoreDetailFragmentViewModel extends BaseViewModel<StoreDetailFragmentNavigator> {

    public StoreDetailFragmentViewModel(IDataManager dataManager, ISchedulerProvider schedulerProvider) {
        super(dataManager, schedulerProvider);
    }


    public void fetchStoreDetail(Activity mActivity, JSONObject jsonObject) {
        if (NetworkUtils.isNetworkConnected(mActivity)) {
            getmNavigator().get().showProgressBars();
            getmDataManger().fetchStoreDetail(mActivity, getmDataManger().getAccessToken(), jsonObject, new ResponseListener() {
                @Override
                public void onSuccess(JsonObject jsonObject) {
                    try {
                        getmNavigator().get().hideProgressBars();
                        getmNavigator().get().onSuccessStoreDetail(jsonObject);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
